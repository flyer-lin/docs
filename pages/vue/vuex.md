...
export default {
  ...
  computed: mapState({
    // 箭头函数可使代码更简练
    // 返回count，等同于count() {return this.$store.state.count}
    count: state => state.count,

    // 传字符串参数 'count' 等同于 `state => state.count`
     // count的别名是countAlias，这时候countAlias等同于count（注意单引号）
    countAlias: 'count',
...
我们会经常用到 ES6 的[参数解构](https://github.com/lukehoban/es6features#destructuring)来简化代码（特别是我们需要调用 `commit` 很多次的时候）：
